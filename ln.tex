...
theorem that there is at least one number $b$
...
\begin{definition}
The \defm{basic symbols} of \textup{\smiley} are ``\textup{\Square}'' and ``\textup{\Circle}''.
\end{definition}

\begin{definition}
An \defm{allowed string} of \textup{\smiley}\ is any finite string of the basic symbols that begins with ``\textup{\Square}''.
\end{definition}
...
\begin{definition}
An allowed string follows by another allowed string by the rule \defm{add-circle} iff it is just like the other string, except with ``\textup{\Circle}'' appended at the end.
\end{definition}
...
\begin{definition}
The set of \defm{favored strings} of \smiley\ is the smallest set containing ``\textup{\Square}'' and closed under add-circle.
\end{definition}
...
\begin{definition}
The \defm{Schmödel number} of an allowed string of \textup{\smiley}\ is the number which is represented in binary notation when ``\,\textup{\Square}'' is interpreted as the digit ``\,$1$'' and ``\textup{\Circle}'' is interpreted as ``\,$0$''.
\end{definition}
...
\item $x$ is the Gödel number of a $y$-place function letter
...
\qquad $x = w \smallfrown z \smallfrown y \bigr)$
...
\item $\mathrm{UnivClosure}(x)$: the Gödel number of the formula resulting from prepending a universal quantifier of the form $∀y$ to the formula with Gödel number $x$ for every variable $y$ it contains free.
...
\end{mt}
\begin{proof}
\Q\ is the closure of the set of axioms Q$_1$--Q$_8$, and the property of being the Gödel number of one of these is simply the disjunction of being any one of them, which is primitive recursive:
